...
export const getAll = async (req: Request, res: Response, next) => {
    try{
        ...
        if (req.query.limit && req.query.page) {
            res.status(200).json(await Appoint.paginate({}, { perPage: limit, page: page, populate: { path: "person" } })) ;
        } else {
            res.status(200).json(await Appoint.find().populate(['person']));
        }
    } catch (error) {
...
export const getOne = async (req: Request, res: Response) => {
    try {
        res.status(200).json(await Appoint.findById(req.params.id));
    } catch (error) {
        res.status(500).json(error);
    }
}
